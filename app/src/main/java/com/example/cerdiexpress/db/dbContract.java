package com.example.cerdiexpress.db;

public class dbContract {
    public static final String DATABASE_NAME = "cerdi_express.db";
    public static final int DATABASE_VERSION = 1;
    public static final String TABLA_PRODUCTO_PEDIDO = "productos_pedido";
    public static final String TABLA_PRODUCTO = "producto";
    public static final String TABLA_PEDIDO = "pedido";
    public static final String COLUMN_ID = "_id";
    public static final String COLUMNA_NOMBRE = "nombre";
    public static final String COLUMNA_CLI_NOMBRE = "cli_nombre";
    public static final String ORDER_ID = "order_id";
    public static final String ORDER_STATUS = "status";
    public static final String COLUMNA_CANTIDAD = "cantidad";
    public static final String COLUMNA_CONTACTO = "contacto";
    public static final String COLUMNA_ORDENANTE = "ordenante";
    public static final String COLUMNA_PRODUCTO = "producto";

    private dbContract() {
    }
}
